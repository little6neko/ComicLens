...
ComicTranslator 的长图切片、OCR、DeepLX 与译文覆写管线集成到阅读器。
...
- OCR、DeepLX、代理、长图参数和阅读默认值均可在 Web 设置页维护。
...
`COMICLENS_OCR_*`、`COMICLENS_DEEPLX_URL` 和 `COMICLENS_PROXY_URL` 只用于首次初始化
尚不存在的服务器设置，之后以数据库中的值为准。
...
- ComicLens 与 Manga18fx、其内容提供者或 OCR/DeepLX 服务提供者无隶属关系。
